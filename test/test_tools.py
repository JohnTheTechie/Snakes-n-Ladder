...
class TestTools(BaseTest):
    ...
    def test_get_next_player(self):
        que: CircularQueue = CircularQueue(3)
        player1 = Player()
        player2 = Player()
        player3 = Player()
        self.assertIsNot(player1, player2)
        que.push_new_player(player1, 2)
        que.push_new_player(player2)
        que.push_new_player(player3)

        foc_pla: CircularQueue.QueueContainer = que.get_next_player()
        self.assertIs(foc_pla.player, player1)
        self.assertIs(foc_pla, que._circular_queue[len(que._circular_queue) - 1])
        self.assertEqual(foc_pla.turns, 2)
        que.complete_transaction()
        self.assertTrue(not que._pending_transaction)
        self.assertEqual(foc_pla.turns, 1)

        foc_pla = que.get_next_player()
        self.assertIs(foc_pla.player, player1)
        self.assertIs(foc_pla, que._circular_queue[len(que._circular_queue) - 1])
        que.complete_transaction()
        self.assertTrue(not que._pending_transaction)
        ...
        foc_pla = que.get_next_player()
        self.assertIsNot(foc_pla.player, player1)
        self.assertIs(foc_pla.player, player2)
        self.assertIsNot(foc_pla.player, player3)
        self.assertIs(foc_pla, que._circular_queue[len(que._circular_queue) - 1])
        que.complete_transaction()
        self.assertTrue(not que._pending_transaction)

        foc_pla = que.get_next_player()
        self.assertIs(foc_pla.player, player3)
        self.assertIs(foc_pla, que._circular_queue[len(que._circular_queue) - 1])
        que.complete_transaction(2)
        foc_pla = que.get_next_player()
        que.complete_transaction()
        foc_pla = que.get_next_player()
        que.complete_transaction()
